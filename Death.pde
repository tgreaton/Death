...
void setup() {
  ...
  data = new FloatTable("deaths-from-homicides-and-suicid.tsv");
...
void drawAxisLabels() {
  ...
  text("Deaths per\n100,000\nin Australia", labelX, (plotY1+plotY2)/2);
  textAlign(CENTER);
...
void drawDataBars(int col) {
  ...
  for (int row = 0; row < rowCount; row++) {
    if (data.isValid(row, col)) {
      ...
      fill(#5679C1);
      if(mouseX > x-barWidth/2 && mouseX < x+barWidth/2 && mouseY > y && mouseY < plotY2){
        fill(0);
        textSize(30);
        textAlign(CENTER);
        text(nf(value, 0, 2) + " (" + years[row] + ")", width/2, y-100);
        textAlign(LEFT);
        fill(255,0,0);
      }
